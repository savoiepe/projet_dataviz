...
def get_figure(data, selected, year):
    ...
    # Ajouter une légende avec le style de ligne
    fig.add_shape(
        type='rect',
        x0=0.05, x1=0.28, xref='paper', y0=0.8, y1=1, yref='paper',
        fillcolor='white', line=dict(color='black', width=1)
    )
    fig.add_shape(
        type='line',
        x0=0.06, x1=0.12, xref='paper', y0=0.95, y1=0.95, yref='paper',
        line=dict(color='red', width=2, dash='dash')
    )
    
    fig.add_annotation(
        text='Début COVID', xref='paper', yref='paper',
        x=0.12, y=0.98, showarrow=False
    )
